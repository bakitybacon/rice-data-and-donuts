...
# filling gaps
djia <- djia %>%
  fill(Date, Close, .direction = "down")
...
```{r diffinter}
difforders = c(1, 7) ## Put your differencing levels here.
...
rnn %>%
  layer_lstm(units=100, return_sequences=TRUE, input_shape = c(num_days_back, 1)) %>%
  layer_activation_relu() %>%
  layer_lstm(units=300, return_sequences=FALSE) %>%
  layer_activation_relu() %>%
  layer_dense(500) %>%
  layer_activation_relu() %>%
  layer_dense(300) %>%
...
ggplot() +
  geom_line(mapping=aes(djia$Date, djia$Close)) +
  geom_line(mapping=aes(djia_test$Date, as.vector(rnnpreds)), color="red") +
  labs(title="A Fitted RNN Model", y="DJIA Close", x="Day")
arimamape <- mean(abs(djia_test$Close -rnnpreds)/djia_test$Close)
print(str_c("MAPE: ", arimamape * 100))
```
